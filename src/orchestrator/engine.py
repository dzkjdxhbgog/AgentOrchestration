...
class OrchestrationEngine:
    def __init__(self, max_workers: int = 10, agent_timeout: int = 300):
        ...
        self.lock_manager = AdvisoryLockManager()
    ...
    async def _execute_task(self, task: Dict[str, Any]) -> None:
        ...
        lock_owner = f"agent:{agent_id}"

        for hook in self._hooks["pre_execute"]:
            await hook(task)

        try:
            await self.lock_manager.run_guarded(
                lock_key,
                lock_owner,
                lambda: self._execute_task_under_lock(agent_id, task),
            )
            logger.info(f"Task {task_id} completed successfully")
        except LockAcquireError as e:
            logger.warning(f"Task {task_id} skipped by lock guard: {e}")
            return
    ...
    async def _execute_task_under_lock(
        self,
        agent_id: str,
        task: Dict[str, Any],
    ) -> Any:
