    ...
    def snapshot(self) -> Dict:
        with self._lock:
            return {
                ...
                "histograms": {k: self._snapshot_histogram(v) for k, v in self._histograms.items()},
            }

    @staticmethod
    def _snapshot_histogram(values: List[float]) -> Dict[str, float]:
        count = len(values)
        total = sum(values)
        return {"count": count, "sum": total, "avg": total / count if count else 0}
