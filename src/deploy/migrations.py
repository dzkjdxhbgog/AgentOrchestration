...
class MigrationSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.backward_compatible, bool):
            raise ValueError(
                f"migration {self.name} must declare backward_compatible "
                "as a boolean"
            )
        if not isinstance(self.required_before_traffic, bool):
            raise ValueError(
                f"migration {self.name} must declare required_before_traffic "
                "as a boolean"
            )
...
class MigrationRolloutGate:
    """Runs required migrations before any new app traffic is allowed."""
    ...
    def check_release_compatibility(
        ...
        for migration in self._migrations:
            ...
            if not migration.required_before_traffic:
                errors.append(
                    f"{migration.name}: must run before traffic shift"
                )
            incompatible = (
                require_backward_compatible
                and not migration.backward_compatible
            )
            if incompatible:
