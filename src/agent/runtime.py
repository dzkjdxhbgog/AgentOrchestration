...
class AgentRuntime:
    def __init__(self):
        ...
        self._terminal_outcomes: Dict[str, TerminalOutcome] = {}
    ...
    def _record_terminal(
        ...
        if existing:
            self._states[agent_id] = existing.state
            return existing
    ...
    def start(self, agent_id: str, command: list, env: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            self._record_terminal(agent_id, RuntimeState.CRASHED, str(e))
    ...
    def stop(
        ...
        if failure_reason:
            self._record_terminal(agent_id, RuntimeState.CRASHED, failure_reason)
        else:
        ...
        if not failure_reason:
            self._record_terminal(agent_id, RuntimeState.STOPPED, "stopped by request")
        logger.info(f"Agent {agent_id} stopped")
        return True

    def get_state(self, agent_id: str) -> RuntimeState:
        terminal = self._terminal_outcomes.get(agent_id)
        if terminal:
        ...
        if proc and proc.poll() is not None:
            ...
            if exit_code == 0:
                self._record_terminal(agent_id, RuntimeState.STOPPED, "process exited", exit_code)
            else:
    ...
    def get_terminal_outcome(self, agent_id: str) -> Optional[TerminalOutcome]:
